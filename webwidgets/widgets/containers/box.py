...
class Box(Container):
    ...
    def build(self):
        ...
        The box is constructed as a `<div>` element with a flexbox layout. The
        flex-direction property is set to either "row" or "column" based on the
        direction parameter, and the element has a data-role attribute of
        "box".

        Each child widget is wrapped inside its own `<div>` element with a
        data-role attribute of "box-item". The items are centered within their
        own `<div>`.
        """
